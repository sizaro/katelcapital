...
export class AccessService {
  ...
  async setRolePermissions(input: SetRolePermissionsInput, actorId: string) {
    ...
    if (role.name === 'SUPER_ADMIN') {
      throw new BadRequestException(
        'The Super Admin role permissions cannot be reduced',
      );
    }
  ...
  async assignUserRole(input: AssignUserRoleInput, actorId: string) {
    const [actor, user, role] = await Promise.all([
    ...
    if (user.role.name === 'SUPER_ADMIN') {
      throw new BadRequestException(
        'The seeded Super Admin role cannot be reassigned',
      );
    }

    if (role.name === 'SUPER_ADMIN' && actor.role.name !== 'SUPER_ADMIN') {
  ...
  private async assertMutableUser(userId: string) {
    ...
    if (user.role.name === 'SUPER_ADMIN') {
      throw new BadRequestException(
        'Direct overrides cannot be applied to the seeded Super Admin',
      );
    }
  }
}
