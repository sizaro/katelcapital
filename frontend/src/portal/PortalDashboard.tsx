...
export default function PortalDashboard({
  ...
  const navigation: NavigationItem[] = [
    { label: "Overview", icon: BriefcaseBusiness, visible: true },
    {
      label: "Academy",
      icon: BookOpen,
      visible: role === "ACADEMY_LEARNER" || hasPermission("academy.view"),
    },
    {
      label: "Professionals",
      icon: Users,
      visible: role === "PROFESSIONAL" || hasPermission("professionals.view"),
    },
    {
      label: "Clients",
      icon: Building2,
      visible: role === "CLIENT" || hasPermission("clients.view"),
    },
    {
      label: "Readiness",
      icon: ShieldCheck,
      visible:
        hasPermission("professionals.approve_ready") ||
        hasPermission("professionals.vet"),
    },
    {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {portalDefinition.label} · {role.replaceAll("_", " ")}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl border px-3 py-2 text-sm hover:bg-slate-100"
            >
      ...
      <div className="mx-auto grid max-w-7xl lg:grid-cols-[250px_1fr]">
        <aside
          className={`${open ? "block" : "hidden"} min-h-[calc(100vh-65px)] border-r bg-white p-4 lg:block`}
        >
        ...
        <main className="p-5 md:p-8">
          <div className="rounded-3xl bg-gradient-to-r from-[#003F8E] to-[#1264b7] p-7 text-white shadow-xl">
            ...
            <h1 className="mt-2 text-3xl font-bold">
              {heading || data?.dashboard.heading || "Your Katel workspace"}
            </h1>
            ...
            <>
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {data?.dashboard.metrics.map((metric, index) => (
                  ...
                  >
                    <div
                      className={`mb-5 h-2 w-12 rounded-full ${index % 3 === 0 ? "bg-[#F7C621]" : index % 3 === 1 ? "bg-[#003F8E]" : "bg-emerald-500"}`}
                    />
